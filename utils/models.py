...
def split_model_repr(model_repr):
    """
    Function to split Django model representation into app label and model name
    """
    # Remove prefix and suffix
    stripped_repr = model_repr.replace("<class '", "").replace("'>", "")
    # Split into app_label and model_name
    app_label, model_name = stripped_repr.split(".")
    ...
    def update_or_create_object(self, data, related_model_map=None):
        ...
        if obj is None:
            if regular_fields:
                ...
                #     obj_model_model = getattr(self.__class__, key).field.related_model
                #     app_label = obj_model_model._meta.app_label
                #     model_name = obj_model_model._meta.model_name
                #     print(f"App label: {app_label}")
                #     print(f"Model name: {model_name}")
                #     obj_model = apps.get_model(app_label, model_name)
